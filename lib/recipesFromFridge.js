...
/**
 * Build a simple meal-plan summary that actually uses saved fridge items when available.
 * @param {Array<{ name: string, count?: number }>} fridgeItems
 * @param {{ calories: number, protein: number }} ctx
 */
export function buildFridgeMealPlanDetails(fridgeItems, ctx) {
    const items = (fridgeItems || [])
    ...
    const breakfastProtein = pickItem(items, [/egg/, /yogurt/, /milk/, /cheese/]);
    const breakfastCarb = pickItem(items, [/oat/, /bread/, /granola/]);
    const lunchProtein = pickItem(items, [/chicken/, /turkey/, /tuna/, /fish/, /salmon/, /tofu/, /bean/], [breakfastProtein]);
    const lunchVeg = pickItem(items, [/spinach/, /lettuce/, /kale/, /broccoli/, /carrot/, /pepper/, /tomato/, /cucumber/, /onion/]);
    const lunchCarb = pickItem(items, [/rice/, /pasta/, /noodle/, /quinoa/, /bread/, /potato/], [breakfastCarb]);
    const dinnerProtein = pickItem(items, [/fish/, /salmon/, /chicken/, /turkey/, /tofu/, /bean/, /egg/], [lunchProtein, breakfastProtein]);
    const dinnerVeg = pickItem(items, [/broccoli/, /spinach/, /kale/, /carrot/, /pepper/, /tomato/, /lettuce/], [lunchVeg]);
    const dinnerCarb = pickItem(items, [/rice/, /quinoa/, /pasta/, /potato/, /noodle/], [lunchCarb]);
    const snackItem = pickItem(items, [/apple/, /banana/, /berry/, /orange/, /grape/, /fruit/, /yogurt/, /milk/, /cheese/], [breakfastProtein, breakfastCarb]);

    const breakfast = formatMeal(
        "Breakfast",
        [
            breakfastProtein && titleCase(breakfastProtein),
            breakfastCarb && titleCase(breakfastCarb),
        ],
        `Use what you have for a balanced start (~${Math.round(ctx.calories * 0.25)} kcal)`
    );

    const lunch = formatMeal(
        "Lunch",
        [
            lunchProtein && titleCase(lunchProtein),
            lunchVeg && titleCase(lunchVeg),
            lunchCarb && titleCase(lunchCarb),
        ],
        `Build a filling plate from your saved items (~${Math.round(ctx.calories * 0.35)} kcal)`
    );

    const dinner = formatMeal(
        "Dinner",
        [
            dinnerProtein && titleCase(dinnerProtein),
            dinnerVeg && titleCase(dinnerVeg),
            dinnerCarb && titleCase(dinnerCarb),
        ],
        `Keep dinner protein-forward and portion-aware (~${Math.round(ctx.calories * 0.3)} kcal)`
    );

    const snack = formatMeal(
        "Snack",
        [snackItem && titleCase(snackItem)],
        `Use a lighter option from your fridge between meals (~${Math.round(ctx.calories * 0.1)} kcal)`
    );
    ...
    return [
        ...
        dinner,
        snack,
        `Total: aim near ${ctx.calories} kcal with ${ctx.protein}g protein (adjust portions to what you have)`,
    ].join(" | ");
}
...
function pickItem(items, patterns, exclude = []) {
    const excluded = new Set((exclude || []).filter(Boolean).map((x) => x.toLowerCase()));
...
function formatMeal(label, pickedItems, fallback) {
    const filtered = pickedItems.filter(Boolean);
    if (!filtered.length) {
        return `${label}: ${fallback}`;
    }
    return `${label}: ${filtered.join(" with ")} (~${label === "Breakfast"
        ? "fridge-based"
        : label === "Snack"
            ? "smart snack"
            : "customized"})`;
