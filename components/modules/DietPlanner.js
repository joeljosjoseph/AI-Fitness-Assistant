...
const DietPlanner = ({ darkMode = false }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if ((!response || !response.ok) && apiBase) { try { response = await fetch(`${apiBase}/diet/predict`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(remoteBody) }); } catch { response = null; } }
